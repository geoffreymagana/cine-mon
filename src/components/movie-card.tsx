...
export const MovieCard = ({ movie, onDelete }: MovieCardProps) => {
  ...
  const handleAddToCollection = (e: React.MouseEvent, collectionId: string) => {
    ...
    try {
      const storedCollections = localStorage.getItem("collections");
      const allCollections: UserCollection[] = storedCollections
  ...
  return (
    <Card
      className="overflow-visible flex flex-col transition-all duration-300 hover:shadow-lg hover:shadow-primary/20 focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 bg-card h-full cursor-pointer"
    ...
    >
      <div className="relative">
        {statusInfo && (
          <Tooltip>
            <TooltipTrigger asChild>
              <div
                className="absolute top-2 left-2 z-20 h-8 w-8 rounded-full bg-background/70 backdrop-blur-sm flex items-center justify-center"
        ...
        <div className="absolute top-2 right-2 z-20" onClick={handleInteraction}>
          <DropdownMenu>
                  ...
                  <DropdownMenuSubContent>
                    {vaults.length > 0 ? (
                      vaults.map((vault) => (
                        <DropdownMenuItem
                          key={vault.id}
                          onClick={(e) => handleAddToCollection(e, vault.id)}
                        >
                          {vault.name}
                        </DropdownMenuItem>
                      ))
                  ...
                  <DropdownMenuSubContent>
                    {spotlights.length > 0 ? (
                      spotlights.map((spotlight) => (
                        <DropdownMenuItem
                          key={spotlight.id}
                          onClick={(e) => handleAddToCollection(e, spotlight.id)}
                        >
                          {spotlight.name}
                        </DropdownMenuItem>
                      ))
